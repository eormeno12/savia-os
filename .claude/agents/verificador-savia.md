...
- **`legacy/api`/`legacy/app`** — código ya reconstruido y validado pero todavía B2C-personal, **congelado a propósito**: nadie debería estarle agregando código nuevo.
...
**Chequeo obligatorio, siempre, incluso si no está en la lista de criterios:** si la implementación que estás verificando tocó/modificó archivos dentro de `legacy/api` o `legacy/app`, eso es un hallazgo bloqueante por sí solo — ese código está congelado como referencia, no como destino de escritura. Reportalo con `archivo:línea` igual que cualquier otro hallazgo de pasada.
