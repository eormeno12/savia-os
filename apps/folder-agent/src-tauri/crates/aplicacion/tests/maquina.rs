...
fn una_raiz_suplantada_cuenta_como_no_viva() {
    ...
    use savia_folder_contrato::dominio::IdDeArchivoDelSO;
    use savia_folder_contrato::plataforma::{EvidenciaDeRaiz, IdDeVolumen, ResultadoDeEnumeracion};
...
fn la_hidratacion_desconocida_se_resuelve_como_no_se_abre() {
    ...
    use savia_folder_contrato::plataforma::Hidratacion;
...
fn la_compactacion_no_cruza_un_borde_de_barrido() {
    ...
    use savia_folder_estado::colas::{Colas, SegmentoId};
    ...
    let h = savia_folder_contrato::hash::sha256(b"v1");
...
fn la_compactacion_deja_uno_por_ruta_dentro_del_segmento() {
    ...
    use savia_folder_estado::colas::{Colas, Encolado};
    let mut c = Colas::nuevas(ParametrosDeCola {
        max_intentos: None,
        max_entradas_por_lote: None,
    });
    ...
    assert_eq!(
        c.encolar(
            &raiz,
            aparicion(r("x.txt"), savia_folder_contrato::hash::sha256(b"v1"))
        ),
        Encolado::Nuevo
    );
    assert_eq!(
        c.encolar(
            &raiz,
            aparicion(r("x.txt"), savia_folder_contrato::hash::sha256(b"v2"))
        ),
        Encolado::Compactado
    );
...
fn los_hechos_van_antes_que_los_bytes() {
    ...
    use savia_folder_estado::colas::{Colas, Desenlace, TrabajoId};
    let mut c = Colas::nuevas(ParametrosDeCola {
        max_intentos: None,
        max_entradas_por_lote: None,
    });
...
fn indeterminada_nunca_produce_hecho_ni_efecto() {
    ...
    // la raiz viva.
    use savia_folder_contrato::plataforma::MotivoIndeterminado;
    let p = Falsa::como_macos();
    p.poner("x.txt", b"algo", 100, Some(1));
    let mut a = almacen();
    p.avanzar(ASENTAMIENTO_DEL_BANCO);
...
fn una_desaparicion_solo_se_construye_pasando_por_la_puerta() {
    ...
    // la puerta.
    use savia_folder_contrato::plataforma::FalloDeEnumeracion;
    use savia_folder_politica::salvaguardas::{EstadoDeRaiz, PorQueAusente, puerta_de_baja};
...
fn una_enumeracion_vacia_no_es_una_raiz_ausente() {
    ...
    use savia_folder_contrato::plataforma::{EvidenciaDeRaiz, ResultadoDeEnumeracion};
...
fn el_brazo_de_windows_corre_en_un_mac_todos_los_dias() {
    ...
    p.poner_con(
        ...
        100,
        Some(1),
        savia_folder_contrato::plataforma::Hidratacion::Deshidratado,
        9,
    );
    assert!(
...
fn la_ruta_relativa_es_canonica_y_no_puede_ser_absoluta() {
    ...
    use savia_folder_contrato::dominio::RutaInvalida;
    assert_eq!(
...
fn el_sha256_coincide_con_el_de_node() {
    ...
    assert_eq!(
        savia_folder_contrato::hash::sha256(b"").hex(),
        "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
    );
    assert_eq!(
        savia_folder_contrato::hash::sha256(b"abc").hex(),
...
fn drenar_anotando(a: &mut Almacen, padron_requerido: bool) -> Vec<String> {
    use savia_folder_estado::colas::Desenlace;
    let mut orden = Vec::new();
    while let Proximo::Trabajo(t) = a.siguiente(&raiz()) {
        let id = match &*t {
            Trabajo::AbrirBarrido { id, .. }
