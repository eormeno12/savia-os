...
// con `Candidato`); todo lo demas de este bloque es de `politica`. Se importa aparte y
// SIN el alias `salvaguardas::` para que no colisione con el modulo de abajo.
    ...
    match plataforma.ficha(&registrada, &senal.ruta) {
        ...
        Ficha::Presente {
            ...
            if let Some(a) = &asiento
                ...
                && misma_observacion(&guardada, &observacion, granularidad)
            {
