...
use savia_folder_contrato::protocolo::{Credencial, Reclamo};
...
use savia_folder_protocolo::{BaseDeApi, Cliente, Tiempos, transporte};
...
fn el_dedupe_previo_a_la_transferencia_ahorra_los_bytes() {
    ...
    let h = savia_folder_contrato::hash::sha256(contenido.as_bytes());
    let ruta = savia_folder_contrato::dominio::RutaRelativa::canonica("x.txt").unwrap();

    let vs = cliente
        .reportar_observados(&raiz, &[(ruta.clone(), h)])
        .expect("presence.observed");
    let permiso = match &vs[0].decision {
        savia_folder_contrato::colas::Decision::Upload { permiso } => permiso.clone(),
        savia_folder_contrato::colas::Decision::Known { .. } => {
...
fn la_divergencia_vuelve_en_el_hash_verificado() {
    ...
    let afirmado = savia_folder_contrato::hash::sha256(b"lo que vi");
    let ruta = savia_folder_contrato::dominio::RutaRelativa::canonica("movil.txt").unwrap();
    let vs = cliente
        .reportar_observados(&raiz, &[(ruta, afirmado)])
        .unwrap();
    let savia_folder_contrato::colas::Decision::Upload { permiso } = &vs[0].decision else {
    ...
    assert_eq!(
        c.verificado.hex(),
        savia_folder_contrato::hash::sha256(b"lo que subi").hex(),
