...
impl Macos {
    ...
    fn recorrer(
        ...
        errores: &mut Vec<super::ErrorDeEntrada>,
    ) {
        let dir = match fs::read_dir(base) {
            Ok(d) => d,
            Err(e) => {
                errores.push(super::ErrorDeEntrada {
        ...
        for entrada in dir {
            ...
            match RutaRelativa::canonica(&relativa) {
                ...
                Err(_) => errores.push(super::ErrorDeEntrada {
                    ruta: None,
                    errno: 0,
                }),
            }
...
impl Plataforma for Macos {
    ...
    fn hashear(
        ...
        let mut estado = crate::hash::Sha256Incremental::nuevo();
        loop {
