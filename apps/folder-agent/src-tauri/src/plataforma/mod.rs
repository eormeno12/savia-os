...
use crate::dominio::{
    HashAfirmado, IdDeArchivoDelSO, Instante, Mtime, Observacion, RaizId, RutaRelativa,
};
...
#[cfg(target_os = "macos")]
pub use macos::Macos;
// SIMETRICO CON `Macos`, y hace falta para que el binario pueda elegir por `cfg` en
// vez de cablear una plataforma. El MODULO es incondicional a proposito —el brazo de
// Windows se compila y se prueba en un Mac todos los dias— pero el ALIAS no puede
// serlo: en macOS resolveria a un tipo cuyos metodos son `unimplemented!()`.
...
/// Constructor de `Mtime` desde el par que devuelven los dos sistemas. Vive aca porque
/// es lo unico que las dos plataformas hacen igual sobre un dato que adquieren
/// distinto.
pub fn mtime_de(segundos: i64, nanos: u32) -> Mtime {
    Mtime { segundos, nanos }
}
