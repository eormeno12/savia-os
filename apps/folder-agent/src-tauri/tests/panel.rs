...
use savia_folder_nucleo::colas::{
    Decision, Desenlace, ParametrosDeCola, Proximo, Recibido, SweepId, Trabajo, Veredicto,
};
use savia_folder_nucleo::dominio::{
    BarridoId, HashVerificado, IdDeArchivoDelSO, RaizId, SensibilidadAMayusculas,
};
use savia_folder_nucleo::panel::{self, EstadoDeArchivo, EstadoDeCarpeta, Motivo};
use savia_folder_nucleo::plataforma::{Falsa, HuellaDeRaiz, IdDeVolumen, RaizRegistrada};
...
fn drenar_con(
    a: &mut Almacen,
    mut responder: impl FnMut(Box<Trabajo>) -> (savia_folder_nucleo::colas::TrabajoId, Recibido),
) {
...
fn las_fotos_de_ejemplo_del_panel_estan_al_dia() {
    ...
    let golden = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../panel/ejemplo.json");
