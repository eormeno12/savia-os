 ...
 *
 * `adapters` cita el plan en cada docstring, igual que `ir`. Esa garantía es una
 * MENTIRA mientras
...
for (const [archivo, ruta] of archivos) {
  ...
  for (const cita of citas) {
    ...
    if (cita.tipo === "rota") {
      errores += 1;
      console.error(
        `ADAPTERS-ERR: ${dónde} — cita mal formada «${cita.crudo}»: el ancla va entre llaves, §{así}.`,
      );
      continue;
    }
    ...
    if (cita.tipo === "número") {
      errores += 1;
      console.error(
        `ADAPTERS-ERR: ${dónde} — cita ${cita.crudo} al plan por número de línea.\n` +
    ...
    if (candidatos.length === 0) {
      errores += 1;
      console.error(`ADAPTERS-ERR: ${dónde} — no hay sección del plan que empiece con «${cita.ancla}»`);
      continue;
    }
    if (candidatos.length > 1) {
      errores += 1;
      console.error(
        `ADAPTERS-ERR: ${dónde} — «${cita.ancla}» es ambigua, ${candidatos.length} secciones responden:\n` +
...
console.log("");
if (errores > 0) {
  console.error(`ADAPTERS-ERR: ${errores} cita${errores === 1 ? "" : "s"} no resuelve al plan.`);
  process.exit(1);
}
