...
// restaurar deja la mutación puesta. Pasó de verdad en `ir` con `turbo lint --continue`.
// Falla en vez de esperar: si hay otra corrida, la respuesta correcta es no correr, no
// encolarse — un arnés que espera parece colgado. El candado vive en el directorio
// temporal del sistema y no adentro del paquete: un archivo suelto en `scripts/` habría
// que ignorarlo en git, y un candado versionado por accidente es peor que no tenerlo.
const CANDADO = join(tmpdir(), `savia-mutants-${Buffer.from(RAIZ).toString("base64url")}.lock`);
if (existsSync(CANDADO)) {
  console.error(
    `ADAPTERS-ERR: ya hay otra corrida de mutantes en este árbol (${CANDADO}).\n` +
