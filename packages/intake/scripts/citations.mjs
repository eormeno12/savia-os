 ...
 * el README opcional. Es la CUARTA copia y ya no es un accidente: cada paquete del
 * pipeline lleva la suya, y el precio de que diverjan está aceptado abajo. Se copia en vez de compartirse porque `intake` no puede
 ...
 *
 * `intake` cita el plan en cada docstring, igual que `ir`. Esa garantía es una
 * MENTIRA mientras
...
for (const [archivo, ruta] of archivos) {
  ...
  for (const cita of citas) {
    ...
    if (cita.tipo === "rota") {
      errores += 1;
      console.error(
        `INTAKE-ERR: ${dónde} — cita mal formada «${cita.crudo}»: el ancla va entre llaves, §{así}.`,
      );
      continue;
    }
    ...
    if (cita.tipo === "número") {
      errores += 1;
      console.error(
        `INTAKE-ERR: ${dónde} — cita ${cita.crudo} al plan por número de línea.\n` +
    ...
    if (candidatos.length === 0) {
      errores += 1;
      console.error(`INTAKE-ERR: ${dónde} — no hay sección del plan que empiece con «${cita.ancla}»`);
      continue;
    }
    if (candidatos.length > 1) {
      errores += 1;
      console.error(
        `INTAKE-ERR: ${dónde} — «${cita.ancla}» es ambigua, ${candidatos.length} secciones responden:\n` +
...
console.log("");
if (errores > 0) {
  console.error(`INTAKE-ERR: ${errores} cita${errores === 1 ? "" : "s"} no resuelve al plan.`);
  process.exit(1);
}
