 ...
 * convención documentada: el tipo `Cuerpo` no tiene ningún miembro capaz de
 * expresarlo».
 *
 ...
 * `verbatim.lenguaje?: string` (§{Tramo 3 › Qué sale}) son identificadores de
 ...
 *
 * PROVISIONAL(Forma): una sola fuente de verdad, y la fuente es `Cuerpo` — El plan
 * nombra `Forma` como tipo propio (§{Dónde vive}) y define los seis discriminantes
 * por separado en `Cuerpo` (§{Tramo 3 › Qué sale}); con dos declaraciones a mano,
 * agregar una variante a `Cuerpo` sin tocar `Forma` deja `TIPO_POR_FORMA`
 ...
 * la da.
 *
 * LA DIVERGENCIA ES IRREPRESENTABLE, no detectable. `Forma` se DERIVA de `Cuerpo`,
 * así que no pueden discrepar; y `satisfies` impide que este arreglo nombre una
 * forma que `Cuerpo` no tiene, en la línea donde se escriba. La versión anterior
 * derivaba `Forma` del arreglo y verificaba la coincidencia con una aserción —
...
export const FORMAS = [
  ...
  "fields",
  "container",
] as const satisfies readonly Cuerpo["forma"][];

export type Forma = Cuerpo["forma"];

// ─────────────────────────────── Tipos colgantes ─────────────────────────────

/**
 * Una marca de estilo inline sobre `text_span.texto`.
 *
 * PROVISIONAL(Marca): conjunto CERRADO de clases, con `[inicio, fin)` en unidades
 * de código UTF-16 sobre el texto YA NORMALIZADO — `Marca` está nombrada como
 * habitante de `ir` (§{Paquetes}) y nunca se define. Cerrado por el mismo argumento
 * con que el plan cerró `tipo` (§{Qué compraba}: «nada impedía que un clasificador
 ...
 */
export type Marca =
  | { readonly clase: "negrita"; readonly desde: number; readonly hasta: number }
  | { readonly clase: "cursiva"; readonly desde: number; readonly hasta: number }
  | { readonly clase: "subrayado"; readonly desde: number; readonly hasta: number }
  | { readonly clase: "tachado"; readonly desde: number; readonly hasta: number }
  | { readonly clase: "codigo"; readonly desde: number; readonly hasta: number }
  | { readonly clase: "super"; readonly desde: number; readonly hasta: number }
  | { readonly clase: "sub"; readonly desde: number; readonly hasta: number }
  | {
      readonly clase: "enlace";
 ...
 * dónde está el encabezado y por lo tanto el `grano`, lo que presupone un sistema
 * de tipos de celda distinto de `Tipo` que el plan nunca define. Excluido de la
 ...
 *
 * PROVISIONAL(H1): `{ texto, tipo }` y no `string` — El plan escribe `filas:
 * Celda[][]` (§{Tramo 3 › Qué sale}) y nunca define `Celda`; que no sea `string` es
 ...
 * (`Celda = string`), `regionesDeGrilla` se queda sin con qué perfilar columnas.
 */
export type Celda = {
  readonly texto: string;
  readonly tipo: TipoCelda | null;
};
 ...
 */
export type Grano = "fila" | "entero";

/**
 * Qué parte de un objeto es una materia descomponible.
 *
 * PROVISIONAL(C4): `Ventana` existe porque `RefObjeto` tiene que dejar de ser una
 ...
 */
export type Ventana =
  | { readonly alcance: "entero" }
  | { readonly alcance: "region"; readonly caja: Caja }
  | { readonly alcance: "rango"; readonly desde: number; readonly hasta: number };

/**
 * La referencia de un `asset`: qué objeto, y qué parte de él.
 * Dos assets con la misma `RefObjeto` hashean igual — que es correcto: son el
 * mismo contenido, y el pase 2 los separa por hueco.
 */
export type RefObjeto = {
  readonly objeto: ClaveObjeto;
 ...
 * campo `pendientes: Enriquecimiento[]` (§{Tramo 3 › Qué sale}) no tiene productor,
 ...
 * RESULTADO va en el `Cuerpo`, R3 lo borra en cada re-ingesta («se regenera entero
 ...
 * `Cuerpo` deja de ser desechable y se rompe R3, que el plan considera de las tres
 * fundamentales.
 *
 ...
 * (§{La delegación es emergente}) y eliminó `tipo:'delegado'` y el clasificador
 * `miembros` por redundantes con ella (§{Lo que se borró}). `pendientes` sobrevivió
 ...
 * este campo sobra.
 */
export type ClaseDeEnriquecimiento = "descripcion" | "ocr" | "transcripcion";

export type Enriquecimiento = {
  readonly clase: ClaseDeEnriquecimiento;
};
 ...
 *  - `container.esquema`  (C3 + C14)
 *  - `grid.encabezados: readonly string[] | null` (era `string[]`)
 *  - `asset.ref: RefObjeto` como par (objeto, ventana) (C4)
 */
export type Cuerpo =
  | {
      readonly forma: "text_span";
      readonly texto: string;
      readonly marcas: readonly Marca[];
    }
  | {
      readonly forma: "verbatim";
      readonly texto: string;
      /**
       ...
       * misma deriva por la que se cerró `tipo` — Si se decide al revés (sin
  ...
  | {
      readonly forma: "asset";
      readonly ref: RefObjeto;
      /**
       ...
       */
      readonly pendientes: readonly Enriquecimiento[];
    }
  | {
      readonly forma: "grid";
      /**
       ...
       * planilla (50 000 identidades vs 1) mientras `encabezados` entra en la
       ...
       */
      readonly encabezados: readonly string[] | null;
      readonly filas: readonly (readonly Celda[])[];
      readonly grano: Grano;
    }
  | {
  ...
  | {
      readonly forma: "container";
      readonly ordenado: boolean;
      /**
       ...
       * `container` es `{forma, ordenado}` — no tiene dónde ponerlo. Las otras
       ...
       */
      readonly esquema: readonly string[] | null;
    };

// La aserción que estaba acá se borró: `Forma` se deriva de `Cuerpo` (ver arriba),
 ...
 * unión.
 */
export type CuerpoDe<F extends Forma> = Extract<Cuerpo, { forma: F }>;
 ...
 *
 * RESIDUO SIN RESOLVER: la canonicalización de `Ventana` (redondeo de cajas)
 * tiene que ser exacta o dos corridas dan `HashMateria` distintos.
 */
export const ventanaCubre = (exterior: Ventana, interior: Ventana): boolean => {
  if (exterior.alcance === "entero") return true;
  if (interior.alcance === "entero") return false;
  if (exterior.alcance === "region" && interior.alcance === "region") {
    return cajaContiene(exterior.caja, interior.caja);
  }
  if (exterior.alcance === "rango" && interior.alcance === "rango") {
    return interior.desde >= exterior.desde && interior.hasta <= exterior.hasta;
  }
  return false;
};

/**
 * Un nodo-FILA: la unidad de identidad de una planilla con `grano: 'fila'`.
 *
 * PROVISIONAL(C3): un nodo-fila es un `grid` con `encabezados: null`, exactamente
 * una fila y `grano: 'fila'` — Es la única lectura que satisface a la vez la
 * medición (renombrar una columna toca UN nodo, porque la huella de la fila son
 * solo sus celdas), el conteo de seis formas cerradas, y el hecho de que `grano`
 * exista en el payload de `grid`. El banco ya lo dice sin darse cuenta: «la huella
 * usaba `texto ?? ref ?? filas`, y una fila no tiene ninguno de los tres»
 ...
 */
export const esNodoFila = (cuerpo: Cuerpo): boolean =>
  cuerpo.forma === "grid" &&
  cuerpo.grano === "fila" &&
  cuerpo.encabezados === null &&
  cuerpo.filas.length === PARAMETROS.aritmética.uno;
